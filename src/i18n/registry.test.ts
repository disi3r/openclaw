...
describe("ui i18n locale registry", () => {
  it("lists supported locales", () => {
    expect(SUPPORTED_LOCALES).toEqual(["en", "zh-CN", "zh-TW", "pt-BR", "es-ES"]);
    expect(DEFAULT_LOCALE).toBe("en");
  });

  it("resolves browser locale fallbacks", () => {
    expect(resolveNavigatorLocale("es-ES")).toBe("es-ES");
  ...
  it("loads lazy locale translations from the registry", async () => {
    const es = await loadLazyLocaleTranslation("es-ES");
